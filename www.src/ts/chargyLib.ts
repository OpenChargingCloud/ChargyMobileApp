...
export default class chargyLib
{
    ...
    public parseOBIS(OBIS: string): string
    {
    ...
    public translateMeasurementName(In: string) : string
    {
        switch (In)
        {
    ...
    public buf2hex(buffer) {
        return Array.prototype.map.call(new Uint8Array(buffer), x => ('00' + x.toString(16)).slice(-2)).join('');
    }

    public intFromBytes(x){
        var val = 0;
    ...
    public SetHex(dv: DataView, hex: string, offset: number, reverse?: boolean): string
    {

        var bytes   = this.parseHexString(hex);
    ...
    public SetTimestamp(dv: DataView, timestamp: any, offset: number): string
    {

        if (typeof timestamp === 'string')
            timestamp = this.parseUTC(timestamp);

        var unixtime  = timestamp.unix();
        var bytes     = this.getInt64Bytes(unixtime);
    ...
    public SetTimestamp32(dv: DataView, timestamp: any, offset: number): string
    {
    ...
    public SetInt8(dv: DataView, value: number, offset: number): string
    {
    ...
    public SetUInt32(dv: DataView, value: number, offset: number, reverse?: boolean): string
    {
    ...
    public SetUInt64(dv: DataView, value: number, offset: number, reverse?: boolean): string
    {

        var bytes   = this.getInt64Bytes(value);
        var buffer  = new ArrayBuffer(bytes.length);
        var tv      = new DataView(buffer);
